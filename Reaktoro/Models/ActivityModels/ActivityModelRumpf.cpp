...
auto ActivityModelRumpf(String gas) -> ActivityModelGenerator
{
    ActivityModelGenerator model = [=](const SpeciesList& species)
    {
        ...
        ActivityModel fn = [=](ActivityPropsRef props, ActivityModelArgs args)
        {
            ...
            const auto& mixture = *std::any_cast<SharedPtr<AqueousMixture> const&>(props.extra["AqueousMixture"]);
            const auto& state = *std::any_cast<SharedPtr<AqueousMixtureState> const&>(props.extra["AqueousMixtureState"]);
